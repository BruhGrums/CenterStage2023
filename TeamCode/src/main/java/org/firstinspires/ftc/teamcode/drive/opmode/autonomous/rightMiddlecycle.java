...
@Config
@Autonomous (name = "Middle cycle test", group = "competition")
public class rightMiddlecycle extends LinearOpMode {
...
// cone is grab
    ...
    private void toStack1(SampleMecanumDrive _drive, int stackHeight ) {
        // stackHeight is given as height of stack in cones
        //step one
...
// cone is grab
    ...
    private void toM(SampleMecanumDrive _drive) {
        //tajectory are blocks so u trajtory finishes before going to the next step
        _drive.updatePoseEstimate();
        TrajectorySequence toMJ = _drive.trajectorySequenceBuilder(stackPose)
                //turn(trun degree, max angal velo , max angle accel)
                .turn(Math.toRadians(-150), Math.toRadians(120), Math.toRadians(90))
                .build ();

        _drive.setHeight(2200);
        _drive.followTrajectorySequence(toMJ);
        _drive.setExtension(300);
        sleep(500);
        _drive.setGrip(false);

    }
    private void toS(SampleMecanumDrive _drive)
    {
        _drive.updatePoseEstimate();
        TrajectorySequence toMJ = _drive.trajectorySequenceBuilder(stackPose)
                //turn(trun degree, max angal velo , max angle accel)
                .turn(Math.toRadians(-15), Math.toRadians(120), Math.toRadians(90))
                .build ();
